...
# This is based on TDNN_OPGRU_1a, but using the vanilla OPGRU to replace the NormOPGRU
# also, to simplify, dropout is also removed from here.
# also, the chunk width is changed from 150 to 150,140,90
# And this is for comparison with TDNN_OPGRU_2b, which uses the fast version of OPGRU.
...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/tdnn_opgru_2a # Note: _sp will get added to this if $speed_perturb == true.
decode_iter=
decode_dir_affix=

# training options
leftmost_questions_truncate=-1
chunk_width=150,140,90
...
if [ $stage -le 15 ]; then
  ...
  [ -z $frames_per_chunk ] && frames_per_chunk=$frames_per_chunk_primary;
  iter_opts=
